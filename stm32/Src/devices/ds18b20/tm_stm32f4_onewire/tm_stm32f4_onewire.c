...
void OneWire_Low(TM_OneWire_t* OneWireStruct) {
	HAL_GPIO_WritePin(OneWireStruct->GPIOx, OneWireStruct->GPIO_Pin, 0);
}
...
void OneWire_Input(TM_OneWire_t* OneWireStruct) {
	...
	for (i = 0x00; i < 0x10; i++) {
		/* Pin is set */
		if (OneWireStruct->GPIO_Pin & (1 << i)) {
			/* Set 00 bits combination for input */
			OneWireStruct->GPIOx->MODER &= ~(0x03 << (2 * i));
		}
	}
}
...
void OneWire_Output(TM_OneWire_t* OneWireStruct) {
	...
	for (i = 0x00; i < 0x10; i++) {
		...
		if (OneWireStruct->GPIO_Pin & (1 << i)) {
			...
			OneWireStruct->GPIOx->MODER = (OneWireStruct->GPIOx->MODER & ~(0x03 << (2 * i))) | (0x01 << (2 * i));
		}
	}
}
