...
#define ONEWIRE_LOW(structure) PORT_LOW(structure)
#define ONEWIRE_HIGH(structure) PORT_HIGH(structure)

//#define ONEWIRE_INPUT(structure) do{\
//GPIO_InitTypeDef GPIO_InitStruct;\
//GPIO_InitStruct.Pin = (structure)->GPIO_Pin;\
//GPIO_InitStruct.Mode = GPIO_MODE_INPUT;\
...
//#define ONEWIRE_INPUT(structure)		do{\
//											uint32_t temp = 0;\
//											temp = (structure)->GPIOx->MODER;\
//											temp &= ~(GPIO_MODER_MODE0 << ((structure)->GPIO_Pin_Pos * 2));\
//											temp |= ((GPIO_MODE_INPUT & ((uint32_t)0x00000003)) << ((structure)->GPIO_Pin_Pos * 2));\
//											(structure)->GPIOx->MODER = temp;\
//										}while(0);
#define ONEWIRE_INPUT(structure) PORT_INPUT(structure)

//#define ONEWIRE_OUTPUT(structure) do{\
//GPIO_InitTypeDef GPIO_InitStruct;\
//GPIO_InitStruct.Pin = (structure)->GPIO_Pin;\
...
//											temp = (structure)->GPIOx->MODER;\
//											temp &= ~(GPIO_MODER_MODE0 << ((structure)->GPIO_Pin_Pos * 2));\
//											temp |= ((GPIO_MODE_OUTPUT_PP & ((uint32_t)0x00000003)) << ((structure)->GPIO_Pin_Pos * 2));\
//											(structure)->GPIOx->MODER = temp;\
//										}while(0);
#define ONEWIRE_OUTPUT(structure) PORT_OUTPUT(structure)
...
typedef struct {
	...
	uint16_t GPIO_Pin_Pos;			/*!< GPIO Pin to be used for I/O functions */
